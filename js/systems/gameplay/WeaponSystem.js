...
export function createWeaponSystem({ state, config, refs, collections, ui, audio, input, entityManager, collisionSystem, callbacks }) {
    ...
    const weaponModelFolder = 'assets';
    ...
    const weaponModelTargetOverrides = {
        Pistol: 0.78,
        Revolver: 0.88,
        'Submachine Gun': 0.95,
        'Assault Rifle': 1.02,
        ...
        'Sniper Rifle': 1.22,
        Shotgun: 1.02,
        'Shotgun Sawed Off': 0.86,
        'Shotgun Short Stock': 0.92
    };
    ...
    function sanitizeWeaponModelName(modelName) {
        if (typeof modelName !== 'string') return null;
        const trimmed = modelName.trim().replace(/\\/g, '/');
    ...
    function getWeaponModelPath(weaponDef) {
        const modelName = sanitizeWeaponModelName(weaponDef?.model);
        if (!modelName) return null;
        return `${weaponModelFolder}/${modelName}.glb`;
    }

    function normalizeWeaponModel(model, weaponDef) {
        ...
        const modelName = sanitizeWeaponModelName(weaponDef?.model);
        const target = weaponModelTargetOverrides[modelName]
            || weaponModelTargets[weaponDef?.type]
            || weaponModelTargets.default;
    ...
    function shoot() {
        ...
        for (let p = 0; p < pellets; p++) {
            ...
            const hit = collisionSystem.raycast(raycaster.ray.origin, raycaster.ray.direction, refs.weapon.range);
            const obstacleDistance = hit ? hit.distance : Infinity;
            const { hit: closestHit, index: closestEnemyIndex } = collisionSystem.raycastEnemies(raycaster, refs.weapon.range, obstacleDistance);
